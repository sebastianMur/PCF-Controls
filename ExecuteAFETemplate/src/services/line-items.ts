...
export const LineItemApi = baseApi.injectEndpoints({
  endpoints: builder => ({
    getLineItem: builder.query<LineItem[], string>({
      query: templateId =>
        `xomuog_lineitems?$select=xomuog_lineitemid,xomuog_defaultunit,xomuog_defaultunitprice,_xomuog_gfcmid_value,xomuog_name&$filter=xomuog_gfcmid/_xomuog_templateid_value eq ${templateId}`,
