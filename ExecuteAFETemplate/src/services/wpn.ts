...
export const wpnApi = baseApi.injectEndpoints({
  endpoints: builder => ({
    getWPN: builder.query<WPN, string>({
      query: wpnId => ({
        url: `xomuog_wellproblemnotifications(${wpnId})?$select=xomuog_wellproblemnotificationid,xomuog_primaryjobtype_ee,_xomuog_engineerid_value,_xomuog_landman_value,xomuog_secondaryjobtype_ee,_xomuog_templateid_value,_xomuog_templatesummaryid_value,_xomuog_wellid_value`,
        headers: {
